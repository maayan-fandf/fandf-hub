...
// The month-picker options ("YYYY-MM" list) are now derived natively from the
// ALL CLIENTS "חודשי" rows via getAvailableMonthsDirect (lib/allClients) — the
// Apps Script `getAvailableMonths` wrapper was retired 2026-07 so the picker no
// longer depends on the report being reachable.
...
export type MorningSignalKind =
  ...
  | "creative-mismatch"
  | "stale-leads"
  // Cross-surface price comparison — generated by Apps Script's
  // morning-feed pass using the LANDING_PRICES tab (populated nightly
  // by a Puppeteer scraper) + Facebook + Google ad copy. Flags
  // projects whose advertised "starting from" price disagrees by
  // >1% across surfaces.
  | "price-mismatch";
