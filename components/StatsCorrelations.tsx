...
export default function StatsCorrelations({
  ...
  highlightProject: string | null;
  compareProject: string | null;
  /** From the page's period multi-select. null/empty = default
   *  "all monthly months" — the scatters sum every YYYY-MM sample
   *  per project. When set, the sums are restricted to those
   *  periods so n + the regression track the picker honestly. */
  selectedPeriods: string[] | null;
}) {
  ...
  const cplCpsStats = useMemo(
    () => buildStats(buildDots(benchmarks, selectedPeriods, "cps")),
    [benchmarks, selectedPeriods],
  );
  const cplCpmStats = useMemo(
    () => buildStats(buildDots(benchmarks, selectedPeriods, "cpm")),
    [benchmarks, selectedPeriods],
  );
  const cplRateStats = useMemo(
    () => buildStats(buildSchedRateDots(benchmarks, selectedPeriods)),
    [benchmarks, selectedPeriods],
  );
