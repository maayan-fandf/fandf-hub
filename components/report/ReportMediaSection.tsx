...
/**
 * פרסום ממומן — the paid-media block (KPI band with threshold framing +
 * prev-window deltas, the CTR-vs-CVR funnel diagnosis, and per-platform
 * cards with campaign-share pies). Ported from the legacy top-funnel
 * section (`renderTopFunnelSection`, Index.html:7983); lives under the
 * קריאייטיבים tab alongside the ad-creative detail. Renders nothing when
 * there is no Supermetrics platform activity in the window.
 */
...
export default function ReportMediaSection({
  ...
  // No paid-platform activity in the window → the block simply doesn't show
  // (the creatives detail below handles its own empty state).
