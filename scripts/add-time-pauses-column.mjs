/* eslint-disable */
// Adds the `time_pauses` column to the Comments tab — a JSON array of
// {at,action,by} pause/resume events on the in-progress time counter.
// While a task is in status `in_progress` the user can ⏸ pause / ▶
// resume the counter (a break, lunch, context-switch — without changing
// status). lib/inProgressTime subtracts paused stretches from the
// status-derived time. Graceful: the writer only persists it if this
// header exists. Idempotent: exits cleanly if the column already
// exists. Mirrors scripts/add-inprogress-minutes-column.mjs.
