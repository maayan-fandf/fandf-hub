/* eslint-disable */
// Adds the `inprogress_minutes` column to the Comments tab — backs the
// editable status-derived time counter. The auto value is the sum of
// every interval the task spent in status `in_progress` (derived from
// status_history); when this column is set it OVERRIDES that auto value
// (so a task left in progress over the weekend can be corrected).
// Graceful: the writer only persists it if this header exists.
// Idempotent: exits cleanly if the column already exists. Mirrors
// scripts/add-price-column.mjs.
...
const NEW_HEADER = "inprogress_minutes";
